#ifndef __graph__getShortestPathSeq_h__

#define __graph__getShortestPathSeq_h__

template<class F>
length_t Graph::getShortestPathSeq(index_t from, index_t to, std::list<Node*>* vals, const F& dist)
{
		...
		if (nl_pos->f <= threshold)
		{
				 ...
				 edge_it != nl_pos->adjEdges.end(); edge_it++)
			{
				...
				if (edge_to->status == Node::inactive)
				{
					...
					edge_to->status = Node::open1;

				} else if (edge_to->status == Node::open1) {
