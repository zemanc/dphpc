...
In our experiments we interpreted the graph as nodes in a 2D plane, because it is easy to visualize. Each node has its position stored, so we can estimate the distance between two nodes with the Euclidean distance or also the Manhattan Distance if we're having a quadratic grid.
...
\mypar{Lock type}
Next to the locks provided by OpenMP we've implemented the following locks
\begin{compactitem}
