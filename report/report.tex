 ...
 \While{x != zero}{
 	\eIf{$x.f \leq threshold$}{
 		\If{$x == end$}{
			...
			return $reconstructPath()$; \tcp*[f]{done} \\
 		}
		...
		\For{each neighbour $nb$}{
			\uIf{$nb.state == open$}{
				...
				\If{$dist < nb.g$}{
					update $nb.f$ and $nb.g$ and set $nb.parent$\;
					move $nb$ right behind $x$ in $nowlaterlist$\;
				}
			}
			\ElseIf{$nb.state = inactive$}{
...
delete $zero$\;
return -1; \tcp*[f]{no existing path to end}
