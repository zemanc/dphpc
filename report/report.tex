...
\caption{Parallel Fringe Search\label{algo:par}. Note: The locking mechanism is not part of this pseudocode, but it's thoroughly described in section \ref{ssec:lock} and in figures \ref{fig:lock}, \ref{fig:insert} and \ref{fig:remove}).}
\end{algorithm}
...
\mypar{Weak scaling}
Because of the limitations in terms of speedup according to Amdahl's law which are also visible in figure \ref{fig:strong_scaling} the next important question is how the runtime behaves with an increasing problem size for an increasing number of cores, the so called weak scaling.\\
First we will look how the runtime of our sequential algorithm behaves with an increasing problem size. The results in figure \ref{fig:seq_scaling} show that the runtime gets considerably higher the bigger the problem is.
\begin{figure}[h]\centering
  \includegraphics[scale=0.558]{seq_scaling.eps}
  \caption{Runtime of sequential algorithm with increasing problem size \label{fig:seq_scaling}}
\end{figure}
Now if we keep the problem size per core (number of nodes per core) constant we still experience an increase in runtime with increasing problem size, bit it's not as dramatic as with the sequential version (see figure \ref{fig:weak_scaling}).
\begin{figure}[h]\centering
  \includegraphics[scale=0.558]{weak_scaling.eps}
  \caption{Weak scaling\label{fig:weak_scaling}}
\end{figure}
The runtime still increases because with an increasing problem size we also increase the sequential part of the algorithm (longer path will lead to more threshold updates).
